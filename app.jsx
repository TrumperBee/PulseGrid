...
const setToast = (callback) => { toastCallback = callback; };
const showToast = (type, message) => { if (toastCallback) toastCallback(type, message); };
...
const Toast = ({ type, message, onClose }) => {
  ...
  return (
    <div className={`toast ${type}`} style={{ position: 'fixed', bottom: '20px', right: '20px', zIndex: 9999 }}>
  ...
  <div className="min-h-screen flex items-center justify-center p-6">
    <div className="max-w-4xl w-full text-center">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center">
        <button onClick={() => onShowAuth('signup')} className="btn-primary px-8 py-4 text-lg">Get Started Free</button>
        <button onClick={() => onShowAuth('login')} className="btn-ghost px-8 py-4 text-lg">Sign In</button>
      </div>
    </div>
  </div>
);
...
const EmailVerificationScreen = ({ email, onContinue, onResend, onBackToLogin, resendCooldown }) => (
  <div className="min-h-screen flex items-center justify-center p-6">
    <div className="card-glass p-8 w-full max-w-md text-center">
      ...
      <div className="space-y-3">
        ...
        <button onClick={onBackToLogin} className="text-gray-400 hover:text-white text-sm mt-4">
          Back to Login
        </button>
      </div>
    </div>
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      try {
        ...
        console.log('Email verification skipped:', verifyError);
      }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (isNewUser) {
        ...
        showToast('success', 'Welcome to PulseGrid! Your account has been created.');
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="card-glass p-8 w-full max-w-md">
          <button onClick={() => { setShowForgotPassword(false); setForgotPasswordSent(false); }} className="text-gray-400 hover:text-white mb-6 flex items-center gap-2">
          ...
          {forgotPasswordSent ? (
            <div className="text-center">
              ...
              <button onClick={() => { setShowForgotPassword(false); setForgotPasswordSent(false); setTab('login'); }} className="btn-primary w-full py-3">
                Back to Login
              </button>
            </div>
          ) : (
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="victor@company.com"
                />
              </div>
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="card-glass p-8 w-full max-w-md">
        <div className="flex gap-2 mb-6">
          <button
            onClick={() => { setTab('login'); setErrors({}); setShowForgotPassword(false); }}
            className={`flex-1 py-2 rounded-lg font-medium transition-colors ${tab === 'login' ? 'bg-cyan-900 text-cyan-400' : 'text-gray-400 hover:text-white'}`}
          >
            Log In
          </button>
          <button
            onClick={() => { setTab('signup'); setErrors({}); setShowForgotPassword(false); }}
            className={`flex-1 py-2 rounded-lg font-medium transition-colors ${tab === 'signup' ? 'bg-cyan-900 text-cyan-400' : 'text-gray-400 hover:text-white'}`}
          >
            Sign Up
          </button>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3 px-4 bg-white text-gray-700 rounded-lg font-medium flex items-center justify-center gap-3 hover:bg-gray-100 transition-colors mb-4"
        ...
        {tab === 'signup' ? (
          <form onSubmit={handleSignUp} className="space-y-4">
            ...
            <p className="text-center text-sm text-gray-400 mt-4">
              ...
              <button type="button" onClick={() => { setTab('login'); setErrors({}); }} className="text-cyan-400 hover:text-cyan-300">
                Log in
              </button>
            </p>
          </form>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="text-right">
              <button type="button" onClick={() => setShowForgotPassword(true)} className="text-sm text-cyan-400 hover:text-cyan-300">
                Forgot password?
              </button>
            </div>
            <button type="submit" disabled={loading} className="btn-primary w-full py-3 flex items-center justify-center gap-2">
            ...
            <p className="text-center text-sm text-gray-400 mt-4">
              ...
              <button type="button" onClick={() => { setTab('signup'); setErrors({}); }} className="text-cyan-400 hover:text-cyan-300">
                Sign up
              </button>
            </p>
          </form>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (!auth.currentUser) {
      pageCallback('landing');
      showToast('info', 'Please log in to access PulseGrid.');
      return;
    }
    ...
    const loadData = async () => {
      try {
        // Load monitors
        const mSnap = await db.collection('monitors')
        ...
        setStats({
          ...
          up,
          down,
          avgResponse: monitorsList.length > 0 ? Math.round(monitorsList.reduce((a, m) => a + (m.avgResponseMs || 0), 0) / monitorsList.length) : 0
        });

        // Load incidents
        const iSnap = await db.collection('incidents')
          .where('userId', '==', auth.currentUser.uid)
          .orderBy('createdAt', 'desc')
          .limit(10)
          .get();
    ...
    // Real-time listener
    const unsubscribe = db.collection('monitors')
      .where('userId', '==', auth.currentUser.uid)
      .onSnapshot((snapshot) => {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => pageCallback('add-monitor')} className="btn-primary flex items-center gap-2">{ICONS.plus} Add Monitor</button>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Monitors', value: stats.total, color: 'cyan', icon: ICONS.activity },
          { label: 'Up', value: stats.up, color: 'green', icon: ICONS.check },
          { label: 'Down', value: stats.down, color: 'red', icon: ICONS.x },
          { label: 'Avg Response', value: stats.avgResponse ? `${stats.avgResponse}ms` : '—', color: 'amber', icon: ICONS.zap },
        ].map((stat, i) => (
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="card-glass p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => pageCallback('monitors')} className="text-sm text-cyan-400">View all</button>
          </div>
          <div className="space-y-3">
...
const AddMonitorPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await db.collection('monitors').add({
        ...
        intervalSeconds: formData.interval,
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <button onClick={() => pageCallback('monitors')} className="flex items-center gap-2 text-gray-400 hover:text-white mb-6">
        {ICONS.chevronLeft} Back
      </button>
              ...
              <tr key={m.id} className="border-t border-gray-800 hover:bg-gray-900/50">
                ...
                <td className="p-4">
                  <div className="flex gap-2">
                    <button onClick={() => handlePause(m.id)} className="p-2 hover:bg-gray-800 rounded" title={m.isPaused ? 'Resume' : 'Pause'}>
                      {m.isPaused ? ICONS.radio : ICONS.pause}
                    </button>
                    <button onClick={() => handleDelete(m.id)} className="p-2 hover:bg-red-900 rounded text-red-400" title="Delete">
                      {ICONS.trash}
                    </button>
                  </div>
                </td>
...
const StatusPagesPage = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = db.collection('statusPages')
      .where('userId', '==', auth.currentUser.uid)
      .orderBy('createdAt', 'desc')
      .onSnapshot((snapshot) => {
  ...
  const createPage = async () => {
    ...
    if (!name) return;
    await db.collection('statusPages').add({
      userId: auth.currentUser.uid,
      name,
          ...
          <nav className="space-y-1">
            {sections.map(s => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${activeSection === s.id ? 'bg-cyan-900/50 text-cyan-400' : 'hover:bg-gray-800 text-gray-400'}`}
              >
                {s.icon}
                <span>{s.label}</span>
              </button>
            ...
            <div>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-gray-900 rounded-lg">
                  ...
                  <div className="font-bold text-cyan-400">{userData?.plan?.toUpperCase() || 'FREE'}</div>
                  <p className="text-xs text-gray-500 mt-1">Pro features enabled during beta</p>
                </div>
                <button onClick={handleLogout} className="btn-ghost text-red-400 hover:text-red-300 flex items-center gap-2">
                  {ICONS.x} Log Out
                </button>
              </div>
            </div>
...
const App = () => {
  ...
  useEffect(() => {
    setToast = (type, message) => setToast({ type, message, onClose: () => setToast(null) });
  ...
  const handleVerifyContinue = async () => {
    ...
    if (auth.currentUser.emailVerified) {
      showToast('success', 'Email verified! Welcome to PulseGrid!');
      setCurrentPage('dashboard');
    } else {
      showToast('error', 'Email not verified yet. Check your inbox and click the link.');
    }
  };
  ...
  const showApp = isLoggedIn && currentPage !== 'landing' && currentPage !== 'login' && currentPage !== 'signup' && currentPage !== 'verify-email';
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg-primary)' }}>
      ...
      {/* Public pages */}
      {currentPage === 'landing' && !isLoggedIn && <LandingPage onShowAuth={(tab) => setCurrentPage(tab)} />}
      {(currentPage === 'login' || currentPage === 'signup') && !isLoggedIn && <AuthPage initialTab={currentPage} />}
      {currentPage === 'verify-email' && <EmailVerificationScreen 
            ...
            <header className="sticky top-0 z-30 bg-black border-b border-gray-800 px-6 py-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-900 border border-cyan-700">
                  <span className="text-xs text-cyan-400">{userData?.plan?.toUpperCase() || 'FREE'}</span>
                </div>
              </div>
            </header>
            <div className="pb-20">
